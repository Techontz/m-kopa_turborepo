...
class SavingController extends Controller
{
    ...
    public function store(Request $request, Customer $customer): RedirectResponse
    {
        ...
        DB::transaction(function () use ($customer, $data, $amount): void {
            ...
            $cash = ['account' => Account::HqSaving, 'branch' => $customer->branch_id];
            $liability = ['account' => Account::SavingsDeposits, 'branch' => $customer->branch_id];
            $data['type'] === 'deposit'
                ? $this->ledger->transfer($customer->company_id, $liability, $cash, $amount, $description, $saving)
                : $this->ledger->transfer($customer->company_id, $cash, $liability, $amount, $description, $saving);
        });
