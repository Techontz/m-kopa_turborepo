...
class HqTransactionTest extends TestCase
{
    ...
    public function test_balance_page_lists_hq_accounts_with_total(): void
    {
        ...
        $ledger->post($admin->company_id, Account::HqSalaryAdvance, 198190, 'Seed');
        $ledger->post($admin->company_id, Account::HqDisbursement, 7184000, 'Seed', date: today()->subDays(10));
    ...
    public function test_transaction_is_requested_then_approved_moving_money(): void
    {
        $admin = $this->signInAdmin();
        $ledger = app(Ledger::class);
        $ledger->post($admin->company_id, Account::HqInterest, 100000, 'Seed');
