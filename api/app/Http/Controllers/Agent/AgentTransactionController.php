...
class AgentTransactionController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($data, $companyId): void {
            ...
            $this->ledger->transfer($companyId, ['account' => Account::Suspense, 'branch' => (int) $data['blanch_id']], ['account' => Account::Agent, 'branch' => (int) $data['blanch_id']], (float) $data['amount'], 'CLIENTLESS TRANSACTION', $transaction, date: CarbonImmutable::parse($data['date']));
        });
