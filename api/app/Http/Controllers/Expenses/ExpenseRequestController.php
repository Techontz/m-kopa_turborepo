...
class ExpenseRequestController extends Controller
{
    ...
    public function accept(AcceptExpenseRequest $request, ExpenseRequest $expenseRequest): RedirectResponse
    {
        ...
        $available = match ($expenseRequest->scope) {
            'branch' => $this->ledger->balance($companyId, Account::Interest, $expenseRequest->branch_id),
        ...
        DB::transaction(function () use ($request, $expenseRequest, $amount, $companyId): void {
            ...
            $source = match ($expenseRequest->scope) {
                ...
                default => ['account' => Account::Company],
            };
            $expense = ['account' => Account::OperatingExpense, 'branch' => $expenseRequest->branch_id, 'expense_type' => $expenseRequest->expense_type_id];
            $this->ledger->transfer($companyId, $source, $expense, $amount, $description, $expenseRequest);
        });
