...
class StaffFinanceTest extends TestCase
{
    ...
    public function test_salary_advance_request_approve_and_reject(): void
    {
        ...
        $this->assertDatabaseHas('ledger_entries', ['account' => Account::Interest->value, 'amount' => -20000, 'branch_id' => $this->staff->branch_id]);
    ...
    public function test_salary_sheet_computes_take_home_and_pays(): void
    {
        ...
        $principalShare = round(4000 * 10000 / 12000, 2);
        $this->assertEqualsWithDelta(-426000 - $principalShare, $this->sum(Account::Interest), 0.01);
        $this->assertEqualsWithDelta($principalShare, $this->sum(Account::Principal), 0.01);
    ...
    {
        return (float) LedgerEntry::where('company_id', $this->admin->company_id)->where('account', $account->value)->sum('amount');
    }
}
