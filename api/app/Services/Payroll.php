...
class Payroll
{
    ...
    public function pay(int $companyId, Account $fromAccount): int
    {
        ...
        DB::transaction(function () use ($rows, $companyId, $fromAccount): void {
            foreach ($rows as $row) {
                ...
                if ($row['take_home'] != 0) {
                    $this->ledger->transfer($companyId, ['account' => $fromAccount, 'branch' => $employee->branch_id], ['account' => Account::SalaryExpense, 'branch' => $employee->branch_id], (float) $row['take_home'], "Salary payment - {$employee->full_name}", $payment);
                }
    ...
    public function repay(StaffLoan $loan, float $amount): StaffLoanPayment
    {
        return DB::transaction(function () use ($loan, $amount): StaffLoanPayment {
            ...
            $this->ledger->journal($loan->company_id, 'Staff loan repayment', [
                ['account' => Account::Principal, 'branch' => $loan->branch_id, 'debit' => $principal],
                ['account' => Account::StaffLoanReceivable, 'employee' => $loan->employee_id, 'credit' => $principal],
                ['account' => Account::Interest, 'branch' => $loan->branch_id, 'debit' => $interest],
                ['account' => Account::InterestIncome, 'branch' => $loan->branch_id, 'credit' => $interest],
            ], $payment, null, $loan->branch_id);
    ...
    private function repayFromSalary(StaffLoan $loan, float $amount, Account $salaryAccount): void
    {
        ...
        $this->ledger->journal($loan->company_id, 'Staff loan restoration from salary', [
            ['account' => Account::SalaryExpense, 'branch' => $loan->branch_id, 'debit' => $principal],
            ['account' => Account::StaffLoanReceivable, 'employee' => $loan->employee_id, 'credit' => $principal],
        ], $payment, null, $loan->branch_id);
    }
