...
class DemoDataSeeder extends Seeder
{
    public function run(Ledger $ledger, LoanService $loans): void
    {
        ...
        $ledger->post($company, Account::Company, 50000000, 'CAPITAL', reference: $capital, date: $today->subMonths(4));
    ...
    private function seedOperations(Company $company, Ledger $ledger, CarbonImmutable $today): void
    {
        ...
        foreach ($branches->take(3) as $index => $branch) {
            ...
            $ledger->post($company, Account::Agent, (float) $transaction->amount, 'AGENT', $branch, $transaction);
        }
        ...
        foreach (Customer::where('company_id', $company->id)->take(5)->get() as $index => $customer) {
            ...
            $ledger->post($company, Account::HqSaving, (float) $saving->amount, 'SAVING DEPOSIT', $customer->branch_id, $saving);
        }
