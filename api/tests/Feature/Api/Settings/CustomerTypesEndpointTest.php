...
class CustomerTypesEndpointTest extends TestCase
{
    ...
    public function test_eligibility_reasons_say_customer_type(): void
    {
        ...
        $type->update(['min_loan_amount' => 100000, 'max_loan_amount' => 500000]);
        $allowed = LoanCategory::factory()->create(['company_id' => $admin->company_id]);
        $other = LoanCategory::factory()->create(['company_id' => $admin->company_id]);
        $type->loanCategories()->sync([$allowed->id]);
        ...
        $violations = $eligibility->violations($customer, $other->id, 50000);
        $this->assertContains('This loan product is not allowed for the customer type', $violations);
        $this->assertContains('Loan amount is below the customer type minimum of 100,000', $violations);
        $this->assertContains('Loan amount exceeds the customer type limit of 500,000', $eligibility->violations($customer->fresh(), $allowed->id, 900000));
