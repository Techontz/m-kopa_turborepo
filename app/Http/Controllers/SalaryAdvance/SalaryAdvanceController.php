...
class SalaryAdvanceController extends Controller
{
    ...
    public function approve(SalaryAdvance $salaryAdvance): RedirectResponse
    {
        ...
        DB::transaction(function () use ($salaryAdvance): void {
            ...
            $this->ledger->post($salaryAdvance->company_id, Account::HqSalaryAdvance, -(float) $salaryAdvance->amount, 'SALARY ADVANCE LOAN', reference: $salaryAdvance);
            if ((float) $salaryAdvance->fee > 0) {
                $this->ledger->post($salaryAdvance->company_id, Account::LoanFee, (float) $salaryAdvance->fee, 'SALARY ADVANCE CHARGER', $salaryAdvance->branch_id, $salaryAdvance);
            }
        });
    ...
    public function destroy(SalaryAdvance $salaryAdvance): RedirectResponse
    {
        DB::transaction(function () use ($salaryAdvance): void {
            if ($salaryAdvance->status !== 'pending') {
                ...
                $this->ledger->post($salaryAdvance->company_id, Account::HqSalaryAdvance, (float) $salaryAdvance->amount - $paid, 'SALARY ADVANCE REMOVED');
                if ((float) $salaryAdvance->fee > 0) {
                    $this->ledger->post($salaryAdvance->company_id, Account::LoanFee, -(float) $salaryAdvance->fee, 'SALARY ADVANCE REMOVED', $salaryAdvance->branch_id);
                }
            }
    ...
    public function pay(Request $request, SalaryAdvance $salaryAdvance): RedirectResponse
    {
        ...
        DB::transaction(function () use ($salaryAdvance, $amount, $remaining): void {
            ...
            $this->ledger->post($salaryAdvance->company_id, Account::HqSalaryAdvance, $amount, 'SALARY ADVANCE DEPOSIT', reference: $payment);
