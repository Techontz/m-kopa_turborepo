...
class FloatApiTest extends TestCase
{
    ...
    public function test_company_float_moves_company_balance_to_branch_principal(): void
    {
        ...
        $this->postJson('/api/v1/capital/floats', ['blanch_amount' => 400000, 'blanch_id' => $admin->branch_id])
            ->assertUnprocessable()->assertJsonValidationErrors('blanch_amount');
        ...
        $this->getJson('/api/v1/capital/floats')->assertOk()->assertJsonPath('total', 400000)->assertJsonPath('data.0.to_branch', Branch::find($admin->branch_id)->name);
    ...
    public function test_account_to_account_within_branch(): void
    {
        ...
        $this->postJson('/api/v1/capital/floats/accounts', ['blanch_id' => $admin->branch_id, 'from_acc' => 'PR', 'to_acc' => 'INT', 'amount' => 1])
            ->assertUnprocessable()->assertJsonValidationErrors('amount');

        $this->postJson('/api/v1/capital/floats/accounts', ['blanch_id' => $admin->branch_id, 'from_acc' => 'INT', 'to_acc' => 'PR', 'amount' => 30000])->assertCreated();
        ...
        $this->getJson('/api/v1/capital/floats/accounts')->assertOk()->assertJsonPath('data.0.from_account', 'INTEREST A/C');
    }
    ...
    public function test_permission_scope_and_isolation(): void
    {
        ...
        $this->postJson('/api/v1/capital/floats', ['blanch_amount' => 1, 'blanch_id' => Branch::factory()->create()->id])->assertUnprocessable()->assertJsonValidationErrors('blanch_id');
